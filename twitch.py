...
class Twitch:
    ...
    def timeout(self, user, timeout_user, timeout_duration):
		self.s.send('PRIVMSG #%s :/timeout %s %d\r\n' % (user, timeout_user, timeout_duration))
        
    def ban(self, user, ban_user):
		self.s.send('PRIVMSG #%s :/ban %s\r\n' % (user, ban_user))
    ...
    def twitch_recieve_messages(self, amount=1024):
        data = None
        try: data = self.s.recv(amount);
        except: return False;
 
        if not data:
            print("Lost connection to Twitch, attempting to reconnect...");
            while True:
			    try: self.twitch_connect(self.user, self.oauth);
			    except: time.sleep(1)
			    else: break
            return None
